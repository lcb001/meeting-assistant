...
func createTemplate(funcType string) prompt.ChatTemplate {
	...
	if funcType == "title" {
		...
		systemMessage = "你是一个{role}，你需要用{style}的语言提炼会议摘要。以json格式返回内容，不要包含换行符，不要将特殊字符转义，至少包括以下内容1. 会议主题 2. 会议参与者3. 会议时间4. 会议内容5. 关键任务提取(这部分必须以以下格式总结，小红的任务是扫地，把其中的人称代词转换成具体的负责人，且需要把相同人们的做的事组合在一起，比如小红的任务是扫地、拖地)，不同负责人的任务使用分号分隔。请注意，直接输出会议摘要，前面不需要前缀（如会议摘要：），会议内容可能会包含一些技术术语和行业术语，你需要根据上下文进行理解和总结。"
